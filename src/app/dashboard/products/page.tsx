...
export default function ProductsPage() {
  ...
  const [editData, setEditData] = useState({ name: '', description: '', price: '', category: '', aliases: '' });

  // New product form
  const [showAdd, setShowAdd] = useState(false);
  const [newProduct, setNewProduct] = useState({ name: '', description: '', price: '', category: '', aliases: '' });
  ...
  const handleAdd = () => {
    ...
    startTransition(async () => {
      ...
      if (res.ok) {
        await fetchProducts();
        setNewProduct({ name: '', description: '', price: '', category: '', aliases: '' });
        setShowAdd(false);
      ...
      {showSync && (
        <div className="bg-card border border-border rounded-2xl p-5 space-y-3 animate-fade-in">
          ...
          <textarea
            ...
            rows={8}
            placeholder={'[{"name": "Product A", "category": "soup", "aliases": ["chanapakh"], "description": "...", "price": 1500}]'}
        ...
        <div className="bg-card border border-primary/30 rounded-2xl p-5 space-y-3 animate-fade-in">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <input value={newProduct.price} onChange={e => setNewProduct(p => ({...p, price: e.target.value}))}
              type="number" placeholder="Price (AMD)" className="bg-background border border-border rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
            <input value={newProduct.category} onChange={e => setNewProduct(p => ({...p, category: e.target.value}))}
              placeholder="Category (e.g. soup, drink)" className="bg-background border border-border rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
            <input value={newProduct.aliases} onChange={e => setNewProduct(p => ({...p, aliases: e.target.value}))}
              placeholder="Aliases (comma-separated)" className="col-span-2 bg-background border border-border rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
          </div>
                ...
                {editId === p.id ? (
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <input value={editData.category} onChange={e => setEditData(d => ({...d, category: e.target.value}))}
                        className="bg-background border border-border rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Category" />
                      <input value={editData.aliases} onChange={e => setEditData(d => ({...d, aliases: e.target.value}))}
                        className="bg-background border border-border rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Aliases (comma-separated)" />
                    </div>
                    <div className="flex items-center gap-2">
                      <input value={editData.price} onChange={e => setEditData(d => ({...d, price: e.target.value}))}
                        type="number" placeholder="Price AMD" className="w-32 bg-background border border-border rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      {p.category && (
                        <span className="inline-block mt-1 text-[10px] uppercase tracking-wide text-primary font-semibold">{p.category}</span>
                      )}
                      {p.description && <p className="text-xs text-muted-foreground mt-0.5 line-clamp-2">{p.description}</p>}
                    </div>
                    <div className="flex items-center gap-3 flex-shrink-0">
                      {p.price != null && (
                        <span className="text-sm font-semibold text-primary">{Number(p.price).toLocaleString()} AMD</span>
                      )}
